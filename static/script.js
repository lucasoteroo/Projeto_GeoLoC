...
function verificarProximidade(latitude, longitude) {
  ...
  if (localizacoesProximas.length > 0) {
    ...
    localizacoesProximas.forEach(localizacao => {
      localizacoesHTML += `<p>Sua localização atual: ${localizacao.name}</p>`;
    });
...
document.addEventListener("DOMContentLoaded", function() {
  // Função para enviar a mensagem automaticamente
  function enviarMensagemAutomatica() {
    var mensagem = "Esta é uma mensagem automática.";
    
    // Criar uma solicitação HTTP para enviar a mensagem
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "{{ url_for('enviar_mensagem') }}", true);
  ...
  // Chamar a função para enviar a mensagem automaticamente
